...
def extract_image_url(payload: dict):
 ...
 if "secureimage" in detail_params:
        sec = detail_params.get("secureimage")
        if isinstance(sec, dict):
            for k in ["value", "origin", "secureImage", "url"]:
                val = sec.get(k)
                if val and isinstance(val, str) and ("http://" in val or "https://" in val):
                    return val
        elif isinstance(sec, str) and ("http://" in sec or "https://" in sec):
            return sec
  return None
app = Flask(__name__)
